...
{
    public class gGenresPageViewModel : BaseViewModel
    {        
        ...
        public gGenresPageViewModel(INavigationService navigationService)
        {
            Title = "Catalog";
        ...
        private async Task LoadMoviesAsync(int page, Enums.MovieCategory movieCategory)
        {
            try
            {
                genres = genres ?? await ApiService.GetGenres().ConfigureAwait(false);
